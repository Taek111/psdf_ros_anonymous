...
class PSDFWrapper(nn.Module):
    ...
    def update_edge_clusters(self, clusters_A, clusters_B, clusters_mask=None):
        ...
        """
        # Type-safe debug logging and empty-list handling
        if isinstance(clusters_A, list):
            K_roi_dbg = len(clusters_A)
            E0_dbg = clusters_A[0].shape[0] if K_roi_dbg > 0 else 0
            # print(f"update_edge_clusters(list): K={K_roi_dbg}, E0={E0_dbg}")
            # Handle empty input by clearing buffers and returning early
            if K_roi_dbg == 0:
                self.clear_clusters()
                self.active_clusters = 0
                return
        else:
            # Assume tensor-like input
            try:
                print(f"update_edge_clusters(tensor): {clusters_A.shape}, {clusters_B.shape}")
            except Exception:
                print("update_edge_clusters: received unsupported types for clusters_A/B")
