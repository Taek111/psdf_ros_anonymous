...
from psdf_ros.core.psdf import PSDF
from psdf_ros.core.psdf_wrapper import PSDFWrapper
from psdf_ros.core.utils import polygon_to_edges

class PSDFOptimizerConfig:
    def __init__(self):
        self.horizon = 20
        self.mat_Q = np.diag([1.0, 1.0, 0.1])
        self.mat_R = np.diag([0.2, 0.5])
        ...
        self.d_safe = 0.01  # minimum safe distance to obstacles
...
class PSDFOptimizer:
    """PSDF-based MPC solver wrapper built on acados."""
    ...
    def create_solver(self):
        ...
        if hasattr(self, 'psdf_model') and self.psdf_model is not None and not isinstance(self.psdf_model, RealTimeL4CasADi):
            # L4CasADi dynamic library 정보를 acados 컴파일러 옵션에 추가
            self.ocp.solver_options.model_external_shared_lib_dir = self.psdf_model.shared_lib_dir
            self.ocp.solver_options.model_external_shared_lib_name = self.psdf_model.name
    ...
    def add_obstacle_avoidance_constraint(self, param, system, obstacles_geo):
        ...
        # Add obstacle avoidance constraint using PSDF model
        if self.psdf_model is not None and self.ocp is not None:
            # Define constraint function: h(x) = sdf(x, y, theta) - d_safe >= 0
            x = self.ocp.model.x  # state variables [x, y, theta]
            
            sdf_value = self.psdf_model(x)
            ...
            p_sym = self.psdf_model.get_sym_params()  # (np, 1)
            ...
            print("Warning: PSDF model not initialized, skipping obstacle avoidance constraint")
    ...
    def setup(self, param, system, reference_trajectory, obstacles):
        ...
        if not self._is_initialized:
            ...
            self.initialize_psdf_model(system, obstacles, E_max=param.max_edges_per_cluster, K_max=param.max_clusters, device="cpu") # Set the PSDF model in the OCP
    ...
    def solve_nlp(self):
        ...
        # Prefer calling through psdf_model; fall back to psdf_wrapper for numeric eval
        try:
            if self.state is not None and getattr(self, 'psdf_model', None) is not None:
                pose_np = np.array(self.state._x, dtype=np.float32)
                sdf_val = None
                # Try calling the real-time L4CasADi wrapper directly (may accept numpy)
                try:
                    res = self.psdf_model(pose_np)
        ...
        # ------------------------------------------------------------------
        if self.psdf_model is not None:
            try:
                N = int(self.N)
                nx = int(self.nx)
            ...
            params_batch = self.psdf_model.get_params(x_guess)  # (N+1, np)
    ...
    def initialize_psdf_model(self, system, obstacles, E_max=100, K_max = 20, device="cpu"):
        """
        ...
        self.psdf_model = RealTimeL4CasADi(self.psdf_wrapper,
