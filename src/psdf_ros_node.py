...
from psdf_ros.core.psdf_optimizer import PSDFOptimizer, PSDFOptimizerConfig
from psdf_ros.core.utils import DifferentialDriveSystem, AckermannSystem, State
from psdf_ros.core.obstacle_detector import line_segments_to_edgeclusters
from psdf_ros.core.ros_params import PsdfRosParams
...
class PSDFRosNode:
    ...
    def __init__(self):
        ...
        self.last_steering = float(self.params.initial_steering)
    ...
    def load_params(self):
        ...
        self.params = PsdfRosParams.from_ros()
    
    def setup_optimizer(self):
        """Initialize the MPC optimizer."""
        # Load optimizer configuration
        cfg_file = self.params.optimizer_config_file
        ...
        cfg.max_clusters = int(self.params.max_clusters)
        cfg.max_edges_per_cluster = int(self.params.max_edges_per_cluster)
        # Emphasize terminal accuracy if requested
        try:
            cfg.terminal_weight = float(self.params.terminal_weight)
        except Exception:
            pass
    ...
    def line_segment_cb(self, msg: LineSegmentList):
        ...
        try:
            ...
            # Determine frames
            source_frame = getattr(msg.header, 'frame_id', '') or self.params.robot_base
            target_frame = self.params.global_frame
    ...
    def apply_velocity_hysteresis(self, v: float) -> float:
        ...
        """
        try:
            eps = float(self.params.v_sign_eps)
            min_interval = float(self.params.v_sign_min_interval)
        except Exception:
    ...
    def handle_service(self, req):
        ...
        try:
            ...
            v_scale = 1.0
            omega_scale = 1.0
            if self.params.approach_slowdown:
                try:
                    dist, yaw_err = self.compute_goal_error(req.current_pose, req.reference_path)
                except Exception:
                    dist, yaw_err = 1e6, math.pi
                r = max(float(self.params.slowdown_radius), 1e-3)
                v_scale = max(float(self.params.min_v_scale), min(1.0, dist / r))
                omega_scale = max(float(self.params.min_omega_scale), min(1.0, dist / r))
            # For Ackermann, only slow down speed near goal; keep steering range intact
            is_ack = (self.params.vehicle_model == 'ackermann')
    ...
    def build_reference_trajectory(self, path: Path, current_state: State) -> np.ndarray:
        ...
        ref_ds_param = float(self.params.ref_ds)
        ...
        try:
            ...
            if not dbg_path.header.frame_id:
                dbg_path.header.frame_id = self.params.global_frame
